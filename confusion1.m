classdef confusion1
    methods (Static)
        ...
        function [c_matrix,Result,RefereceResult,predict,actual]= getMatrix(actual,predict,m,Display)
            actual=sort(actual);
            
            [v,~]=standardize(actual);
            ...
            disp(c_matrix_table)
            [Result,RefereceResult]=confusion1.getValues(c_matrix);
            ...
            function [value,mu]=standardize(value) 
            per=@f;
            cv = cvpartition(value,'HoldOut',per(m));
            idx = cv.test;
            mu = value(~idx,:);
            v=shuffle(value);
            value(idx,:)=v(idx,:);
